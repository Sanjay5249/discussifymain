...
export const createPost = async (req, res) => {
    try {
        let { content, communityId, linkUrl, type } = req.body;
        const author = req.user._id;

        // Validation - communityId is required, content is optional if files or link are provided
        if (!communityId) {
            return res.status(400).json({ success: false, message: 'Community ID is required.' });
        }

        // Either content, files, or link must be provided
        const hasFiles = (req.files && req.files.length > 0) || req.file;
        if (!content && !hasFiles && !linkUrl) {
            return res.status(400).json({ success: false, message: 'Content, files, or link are required.' });
        }
        ...
        // Handle link posts
        if (linkUrl) {
            newPostData.type = 'link';
            newPostData.linkUrl = linkUrl;
        }
