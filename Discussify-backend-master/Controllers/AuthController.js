...
export const register = async (req, res) => {
  try {
    ...
    if (!username || !email || !password || !interests) {
      ...
      return res.status(400).json({
        success: false,
        message: 'Please provide username, email, password, and interests'
      });
    }
    ...
    if (email.length > 254) {
      if (req.file) await fs.unlink(req.file.path);
      return res.status(400).json({ success: false, message: 'Email address is too long (max 254 characters).' });
    }

    if (email.split('@')[0].length > 64) {
      if (req.file) await fs.unlink(req.file.path);
      return res.status(400).json({ success: false, message: 'Email local part is too long (max 64 characters).' });
    }

    if (/\.{2,}/.test(email)) {
      if (req.file) await fs.unlink(req.file.path);
      return res.status(400).json({ success: false, message: 'Email cannot contain consecutive dots.' });
    }
...
export const getMe = async (req, res) => {
  try {
    ...
    if (req.user._id === MOCK_ADMIN_ID || req.user._id.toString() === MOCK_ADMIN_ID) {
      return res.status(200).json({
        success: true,
        data: {
          _id: MOCK_ADMIN_ID,
...
export const updateProfile = async (req, res) => {
  try {
    ...
    // Mock admin - save profile image
    if (userId === MOCK_ADMIN_ID || userId.toString() === MOCK_ADMIN_ID) {
      let adminProfileImage = null;

      // Save the uploaded image for admin
      if (req.file) {
        adminProfileImage = `uploads/${req.file.filename}`;
      }
      ...
      return res.status(200).json({
        ...
        user: {
          id: MOCK_ADMIN_ID,
          _id: MOCK_ADMIN_ID,
          username: username || 'DiscussifyAdmin',
          email: 'admin@discussify.com',
          bio: bio || 'Master Administrator',
          profileImage: adminProfileImage,
          interests: interests || ['Technology', 'Education'],
          isEmailVerified: true,
          role: 'admin'
        }
      });
    ...
    // Update interests
    if (interests) {
      console.log('Received interests:', interests, 'Type:', typeof interests, 'IsArray:', Array.isArray(interests));
      let userInterests = [];
    ...
    if (req.body.notificationSettings) {
      let settings = req.body.notificationSettings;
      ...
      const validKeys = ['communityInvites', 'communityUpdates', 'postNotifications', 'commentReplies', 'welcomeMessages', 'emailNotifications'];
